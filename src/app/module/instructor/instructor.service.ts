...
const updateInstructor = async (email: string, payload: Partial<Instructor>, file?: any) => {
  // 1️⃣ File upload (optional)
  if (file) {
    const { secure_url }: any = await uploadToCloudinary(`${email}-${payload.fullName}`, file.buffer);
    payload.image = secure_url;
  }

  const updatedInstructor = await prisma.$transaction(async (tx) => {
    // 2️⃣ Find existing instructor
    const existingInstructor = await tx.instructor.findUnique({ where: { email } });
